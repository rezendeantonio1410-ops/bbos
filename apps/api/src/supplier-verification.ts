...
export function validateStateRegistration(value: string | null | undefined, state: string | null | undefined): boolean {
  if (!value?.trim()) return true;
  const digits = onlyDigits(value);
  const expectedLength: Record<string, number> = { PR: 10, SP: 12, MG: 13, ES: 9 };
  const length = expectedLength[state ?? ""];
  return length ? digits.length === length : digits.length >= 5 && digits.length <= 14;
}
