...
export class CustomerNotificationService {
  ...
  private async sendEmail(row: any) {
    ...
    if (!apiKey || !from) throw new Error("RESEND_API_KEY/STOREFRONT_FROM_EMAIL não configurados.");
    ...
    if (!response.ok) throw new Error(`Resend recusou a notificação (${response.status}).`);
  ...
  async processNext() {
    ...
    try {
      if (row.channel !== "EMAIL") throw new Error(`Canal ainda não configurado: ${row.channel}`);
      ...
      const retryMinutes = Math.min(120, Math.max(5, 2 ** Math.min(Number(row.attempts || 1), 6)));
      await this.database.$executeRawUnsafe(
        ...
        row.id, message.slice(0, 2000), String(retryMinutes),
      );
