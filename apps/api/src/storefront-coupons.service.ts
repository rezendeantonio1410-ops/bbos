...
export class StorefrontCouponsService {
  ...
  async calculate(companyId: string, rawCode: unknown, subtotalCents: number): Promise<CouponCalculation> {
    ...
    if (coupon.usageLimit != null && Number(coupon.usageCount) >= Number(coupon.usageLimit))
      throw new BadRequestException("Este cupom atingiu o limite de utilizações.");
    if (subtotalCents < Number(coupon.minimumSubtotalCents || 0))
      throw new BadRequestException("O valor mínimo deste cupom ainda não foi atingido.");
    ...
    const commissionBase = coupon.commissionBasis === "GROSS_SUBTOTAL" ? subtotalCents : netSubtotalCents;
    const commissionCents = coupon.commissionType === "PERCENT"
      ? Math.round(commissionBase * Number(coupon.commissionValue) / 100)
      : Math.round(Number(coupon.commissionValue) * 100);
    return {
      id: coupon.id,
      code,
      brokerId: coupon.brokerId,
