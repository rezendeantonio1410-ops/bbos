...
function stepCells(eventType: string) {
  ...
  return steps
    .map((label, index) => {
      ...
      const dot = complete ? "#087568" : active ? "#C8923E" : "#0E191D";
...
export function renderCustomerEmail(
  ...
  const orderSummary = items.length
    ? `<tr><td style="padding:42px 42px 0"><div style="font-family:Arial,sans-serif;font-size:10px;line-height:14px;letter-spacing:2px;text-transform:uppercase;color:#0E191D">Sua escolha</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:18px">${itemRows(items)}<tr><td colspan="2" style="border-top:1px solid #E0EAE9"></td></tr></table></td></tr>
      <tr><td style="padding:28px 42px 0"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#EDF3F1;border-left:3px solid #087568"><tr><td style="padding:20px 22px"><div style="font-family:Arial,sans-serif;font-size:10px;line-height:14px;letter-spacing:1.7px;text-transform:uppercase;color:#087568">Uma nota do Bispo</div><div style="padding-top:8px;font-family:Georgia,'Times New Roman',serif;font-size:15px;line-height:23px;font-style:italic;color:#0E191D">${escapeHtml(note)}</div></td></tr></table></td></tr>
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(payload.title || "Seu pedido Bispo")}</title></head><body style="margin:0;padding:0;background:#F1EEE8;color:#0E191D"><div style="display:none;max-height:0;overflow:hidden;opacity:0">${escapeHtml(payload.detail)}&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#F1EEE8"><tr><td align="center" style="padding:24px 10px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#FFFFFF">
    <tr><td style="height:4px;background:#087568;font-size:0;line-height:0">&nbsp;</td><td style="height:4px;background:#C8923E;font-size:0;line-height:0">&nbsp;</td><td style="height:4px;background:#E0EAE9;font-size:0;line-height:0">&nbsp;</td></tr>
    ...
    <tr><td colspan="3" style="padding:43px 42px 38px;background:#0A0A0A;color:#FFFFFF"><div style="font:10px/14px Arial,sans-serif;letter-spacing:2px;text-transform:uppercase;color:#C8923E">${escapeHtml(payload.title || "Seu pedido Bispo")}</div><h1 style="margin:18px 0 17px;font-family:Georgia,'Times New Roman',serif;font-size:36px;line-height:42px;font-weight:400;color:#FFFFFF">Seu café já começou<br>a jornada.</h1><div style="font:14px/23px Arial,sans-serif;color:#E0EAE9">${escapeHtml(customerFirstName)}, ${escapeHtml(payload.detail || "acompanharemos cada etapa até o café chegar a você.")}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:27px;border-top:1px solid #3E484B"><tr><td style="padding-top:16px;font:11px/16px Arial,sans-serif;letter-spacing:.8px;text-transform:uppercase;color:#FFFFFF">${escapeHtml(payload.orderCode)}</td><td align="right" style="padding-top:16px;font:11px/16px Arial,sans-serif;color:#AEB8B6">Bispo Coffees</td></tr></table></td></tr>
    ...
    <tr><td colspan="3"><table role="presentation" width="100%" cellspacing="0" cellpadding="0">${orderSummary}${tracking}<tr><td style="padding:0 42px 32px;text-align:center;font:11px/18px Arial,sans-serif;color:#626B69">Se precisar, responda a este e-mail.<br>Nossa equipe cuidará de você.</td></tr></table></td></tr>
    <tr><td colspan="3" style="padding:25px 42px;background:#E0EAE9"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td style="font:500 13px Arial,sans-serif;letter-spacing:2px;color:#0E191D">BISPO</td><td align="right" style="font:9px/15px Arial,sans-serif;letter-spacing:.4px;text-transform:uppercase;color:#52605D">True Coffee · Londrina, Paraná<br>Bispo Coffees Ltda · CNPJ 13.008.726/0001-12</td></tr></table></td></tr>
