...
type Session = { id: string; code: string; kind: "PROFESSIONAL" | "TRAINING"; startedAt?: string | null; sample?: { code: string; supplier?: { name: string } | null; originUnit?: { name: string } | null; harvest?: string | null; species?: string | null; cultivar?: string | null; process?: string | null } | null };
...
export default function PublicCuppingSessionPage() {
  ...
  useEffect(() => { if (!token || !session || session.startedAt) return; const timer = window.setInterval(() => { void request<Session>(`${API}/${token}`).then(setSession).catch(() => undefined); }, 5000); return () => window.clearInterval(timer); }, [session, token]);
  useEffect(() => {
