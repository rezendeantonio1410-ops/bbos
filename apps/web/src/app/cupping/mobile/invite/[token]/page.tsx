...
export default function InvitePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/cupping-public/public/${token}/mobile`)
      .then(async (response) => { if (!response.ok) throw new Error("Este convite não está disponível."); return response.json() as Promise<{ sessionId: string }>; })
      .then(({ sessionId }) => router.replace(`/cupping/mobile/session/${sessionId}?invite=${encodeURIComponent(token)}`))
