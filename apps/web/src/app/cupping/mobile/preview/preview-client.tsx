...
export default function PreviewClient() {
  ...
  const completeSensory = () => { const message = current.id === "fragrance" || current.id === "aroma" ? "Fragrância + Aroma registrada ✓" : `${current.label} registrado ✓`; const nextStage = stages[index + 1]?.id ?? current.id; setFeedback(message); setStage(nextStage); setSensoryDepth(0); window.setTimeout(() => setFeedback(null), 1000); };
