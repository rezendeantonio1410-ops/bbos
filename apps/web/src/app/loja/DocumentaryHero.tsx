...
const scenes:Scene[]=[
  ...
  {kind:"photo",src:"/WhatsApp Image 2026-09-01 at 21.36.46 (1).jpeg",seconds:2.8,label:"origem"},
  {kind:"video",src:"/WhatsApp Video 2026-09-01 at 15.05.46.mp4",start:1,seconds:3.4,label:"prova"},
  {kind:"video",src:"/WhatsApp Video 2026-09-01 at 15.05.42.mp4",start:1,seconds:3.4,label:"escolha"}
];
const fallback:Scene={kind:"video",src:"/WhatsApp Video 2026-09-02 at 17.34.18.mp4",start:1,seconds:4.2};
const poster="/WhatsApp Image 2026-09-02 at 17.53.29.jpeg";
...
export default function DocumentaryHero(){
  ...
  return <div className={styles.authorityVisual}><div className={media.frame}>
    <img src={poster} alt="Café Bispo Coffees" className={`${media.photo} ${!ready?media.ready:""}`}/>
    {current.kind==="video"?<video key={current.src} className={`${media.film} ${ready?media.ready:""}`} autoPlay muted playsInline preload="metadata" poster={poster}
      ...
      onEnded={next} onError={next} aria-label={`Bispo Coffees${current.label?` — ${current.label}`:""}`}><source src={current.src} type="video/mp4"/></video>
    :<img key={current.src} src={current.src} alt={`Bispo Coffees — ${current.label??"origem"}`} className={`${media.film} ${ready?media.ready:""}`} onLoad={()=>{setReady(true);schedule(current.seconds)}}/>}
    <div className={`${media.veil} ${ready?media.ready:""}`}/>{current.label&&<div className={media.words}><span>{current.label}</span></div>}<div className={media.counter}>{String(scene+1).padStart(2,"0")} / {String(scenes.length).padStart(2,"0")}</div>
  </div></div>
}
