  ...
  return <main className={styles.page}>
    ...
    <section id="top" className={`${styles.hero} ${tight.hero}`}>
      ...
      <div className={`${styles.heroCopy} ${tight.heroCopy}`}>
        ...
        <div className={tight.productDesireStage} data-photo-slot="hero-product-real-caramelo" aria-label="Espaço reservado para a fotografia real do café Caramelo">
          <div className={tight.productAura}/><div className={tight.productShot}><div className={tight.productPlaceholder}><small>PRODUTO REAL</small><b>CARAMELO</b><span>foto entra aqui</span></div></div>
          <div className={`${tight.flavorWhisper} ${tight.flavorTop}`}><b>caramelo</b><span>doçura envolvente</span></div><div className={`${tight.flavorWhisper} ${tight.flavorMiddle}`}><b>chocolate</b><span>corpo presente</span></div><div className={`${tight.flavorWhisper} ${tight.flavorBottom}`}><b>equilíbrio</b><span>acidez delicada</span></div>
        </div>
    ...
    <section id="cafes" className={styles.productsSection}>
      <div className={styles.sectionHeader}><div><small>CAFÉS BISPO</small><h2>Você sente primeiro.<br/>Depois escolhe o nome.</h2></div><p>Perfis claros e produtos reais para você reconhecer o que gosta — e saber o que esperar da próxima xícara.</p></div>
      <div className={styles.productGrid}>{products.map(p=><article key={p.name} className={styles.productCard} style={{"--tone":p.tone} as React.CSSProperties}><div className={styles.productVisual} data-photo-slot={`product-${p.name.toLowerCase().replaceAll(" ","-")}`}>{p.image&&<img src={p.image} alt={`Embalagem Bispo ${p.name}`} className={styles.productPhoto}/>}<small>{p.tag}</small></div><div className={styles.productMeta}><p>{p.line}</p><h3>{p.name}</h3><span>{p.notes}</span><div className={styles.buyRow}><strong>{p.price} <small>· {p.weight}</small></strong><button aria-label={`Escolher ${p.name}`}>Quero esse →</button></div></div></article>)}</div>
