  ...
  return <main className={styles.page}>
    ...
    <section id="top" className={`${styles.hero} ${tight.hero}`}>
      ...
      <div className={`${styles.heroCopy} ${tight.heroCopy}`}>
        ...
        <div className={tight.sensoryStage} aria-label="Experiência sensorial do café Caramelo" data-photo-slot="hero-product">
    ...
    <section id="escolher" className={styles.choiceSection}>
      <div className={styles.choiceIntro}><small>COMECE PELA VONTADE</small><h2>O que você quer sentir hoje?</h2><p>Você não precisa entender de café para escolher muito bem.</p></div>
    ...
    <section id="cafes" className={styles.productsSection}>
      <div className={styles.sectionHeader}><div><small>CAFÉS BISPO</small><h2>Escolha com os olhos.<br/>Confirme com o paladar.</h2></div><p>Perfis claros para você reconhecer o que gosta e voltar a encontrar.</p></div>
    ...
    <section className={styles.desireBand}>
      <div><small>POR QUE BISPO</small><h2>O conhecimento fica com a gente.<br/>A escolha fica simples para você.</h2></div>
      <div className={styles.valueCards}><span><b>Selecionado na origem</b><small>Escolhemos cafés pelo que realmente entregam na xícara.</small></span><span><b>Torra e prova</b><small>Cuidamos para preservar identidade, doçura e equilíbrio.</small></span><span><b>Perfil reconhecível</b><small>Gostou? Você sabe o que procurar quando quiser sentir aquilo de novo.</small></span></div>
    </section>
    ...
    <section className={styles.rareSection}><div><small>RAROS · A EXCEÇÃO</small><h2>Alguns cafés não foram feitos para se repetir.</h2><p>Microlotes para quando você quer sair do conhecido e provar algo realmente extraordinário.</p></div><a href="#cafes">Quero ser surpreendido →</a></section>
    ...
    <section className={styles.institutional} data-photo-slot="brand-story">
      ...
      <div><p><b>José Rezende, o Bispo,</b> construiu sua trajetória entre produção, prova, qualidade e mercado de cafés brasileiros.</p><p><b>Suzi Ninov</b> soma uma história próxima de produtores, lavoura e transformação no campo.</p><Link href="/loja/sobre">Conheça a história completa →</Link></div>
    </section>
