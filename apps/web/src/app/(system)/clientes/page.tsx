...
type CreditEvent = {
  ...
  decisionStatus?: Customer["creditStatus"] | null;
  decidedLimit?: string | number | null;
  decisionTerms?: string | null;
  notes?: string | null;
  actorName: string;
...
const segments = ["Distribuidor","Cafeteria","Escritório","Varejo","Restaurante / Hotel","White Label","Exportação","Consumidor final","Outro"];
const states = ["AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG","PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO"];
const paymentOptions = ["À vista", "7 dias", "14 dias", "21 dias", "28 dias", "30 dias", "45 dias", "60 dias", "Personalizada"];
const emptyCustomer = { name:"",legalName:"",tradeName:"",taxId:"",segment:"",email:"",phone:"",postalCode:"",address:"",district:"",city:"",state:"",paymentTerms:"" };
const creditLabel = { NOT_ANALYZED:"Não analisado", UNDER_REVIEW:"Em análise", APPROVED:"Aprovado", REJECTED:"Reprovado" };
...
export default function CustomersPage() {
  ...
  const [requestCustomer,setRequestCustomer] = useState<Customer|null>(null);
  const [requestedLimit,setRequestedLimit] = useState("");
  const [requestedTerms,setRequestedTerms] = useState("28 dias");
  ...
  const [creditTerms,setCreditTerms] = useState("28 dias");
  ...
  const load = async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível carregar clientes."); }
  ...
  const openRequest = (customer:Customer) => {
    setError("");
    setRequestCustomer(customer);
    setRequestedLimit(customer.creditStatus==="APPROVED" ? String(customer.creditLimit??"") : "");
    setRequestedTerms(customer.paymentTerms && customer.paymentTerms!=="À vista" ? customer.paymentTerms : "28 dias");
    setMonthlyVolume(""); setRequestRationale("");
  };

  const submitCreditRequest = async () => {
    if(!requestCustomer) return;
    setSaving(true); setError("");
    try {
      const response=await fetch(`/api/customers/${requestCustomer.id}/credit-request`,{method:"POST",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({requestedLimit:Number(requestedLimit||0),requestedTerms,monthlyVolume:monthlyVolume===""?undefined:Number(monthlyVolume),rationale:requestRationale})});
      const payload=await response.json().catch(()=>({}));
      if(!response.ok) throw new Error(payload?.message ?? "Não foi possível enviar a solicitação de crédito.");
      setRequestCustomer(null); await load();
    } catch(cause){setError(cause instanceof Error?cause.message:"Não foi possível enviar a solicitação de crédito.");}
    finally{setSaving(false);}
  };
  ...
  const fetchHistory = async (customerId:string) => {
    ...
    try {
      const response=await fetch(`/api/customers/${customerId}/credit-history`,{credentials:"include",cache:"no-store"});
      setCreditHistory(response.ok ? await response.json() : []);
    } catch { setCreditHistory([]); }
    finally { setHistoryLoading(false); }
  };

  const openCredit = (customer:Customer) => {
    setError(""); setCreditCustomer(customer);
    ...
    setCreditTerms(customer.paymentTerms && customer.paymentTerms!=="À vista" ? customer.paymentTerms : "28 dias");
    setCreditNotes(customer.creditNotes??"");
    void fetchHistory(customer.id);
  };
  ...
  const saveCredit = async () => {
    ...
    setSaving(true); setError("");
    try {
      const response=await fetch(`/api/customers/${creditCustomer.id}/credit`,{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({status:creditStatus,creditLimit:Number(creditLimit||0),paymentTerms:creditTerms,notes:creditNotes})});
      const payload=await response.json().catch(()=>({}));
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[.18em] text-violet-700"><Sparkles size={13}/> Comercial inteligente</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Clientes</h1><p className="mt-1 max-w-2xl text-sm text-stone-500">O BBOS organiza o cadastro, lê a saúde financeira e ajuda a decidir o próximo passo.</p></div>
    ...
    {requestCustomer&&<div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4"><div className="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl">
      <div className="flex items-start justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-emerald-700">Etapa 1 · Solicitação</p><h2 className="mt-1 text-2xl font-semibold">Solicitar crédito</h2><p className="mt-1 text-sm text-stone-500">{requestCustomer.tradeName||requestCustomer.name}</p></div><button onClick={()=>setRequestCustomer(null)} className="rounded-xl border p-2"><X size={18}/></button></div>
      <div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><div className="flex gap-3"><History size={18} className="mt-0.5 text-emerald-700"/><div><p className="text-sm font-semibold text-emerald-950">Esta solicitação entra na trilha de auditoria</p><p className="mt-1 text-xs leading-5 text-emerald-800">O BBOS registra quem solicitou, quando, valor, condição e justificativa. A decisão administrativa será registrada separadamente.</p></div></div></div>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <Field label="Limite solicitado (R$)"><input type="number" min="0" step="0.01" value={requestedLimit} onChange={e=>setRequestedLimit(e.target.value)} placeholder="Ex.: 50000"/></Field>
        <Field label="Condição solicitada"><select value={requestedTerms} onChange={e=>setRequestedTerms(e.target.value)}>{paymentOptions.filter(p=>p!=="À vista").map(p=><option key={p}>{p}</option>)}</select></Field>
        <Field label="Volume mensal estimado (R$)" help="Opcional. Ajuda a administração a dimensionar o limite." wide><input type="number" min="0" step="0.01" value={monthlyVolume} onChange={e=>setMonthlyVolume(e.target.value)} placeholder="Ex.: 20000"/></Field>
        <Field label="Justificativa da solicitação" wide><textarea rows={4} value={requestRationale} onChange={e=>setRequestRationale(e.target.value)} placeholder="Ex.: início de fornecimento recorrente, volume esperado, histórico comercial, referências..."/></Field>
      </div>
      {error&&<div className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}
      <button disabled={saving} onClick={()=>void submitCreditRequest()} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50"><Send size={16}/>{saving?"Enviando…":"Enviar para análise"}</button>
    </div></div>}

    {creditCustomer&&<div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl">
      <div className="flex items-start justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-violet-700">Etapa 2 · Decisão administrativa</p><h2 className="mt-1 text-2xl font-semibold">Análise de crédito</h2><p className="mt-1 text-sm text-stone-500">{creditCustomer.tradeName||creditCustomer.name}</p></div><button onClick={()=>setCreditCustomer(null)} className="rounded-xl border p-2"><X size={18}/></button></div>
      <HealthPanel health={creditCustomer.financialHealth} compact/>
      <CreditRequestContext history={creditHistory} loading={historyLoading}/>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <Field label="Situação"><select value={creditStatus} onChange={e=>setCreditStatus(e.target.value as Customer["creditStatus"])}><option value="UNDER_REVIEW">Em análise</option><option value="APPROVED">Aprovado</option><option value="REJECTED">Reprovado</option><option value="NOT_ANALYZED">Não analisado</option></select></Field>
        ...
        <Field label="Condição aprovada" wide><select value={creditTerms} onChange={e=>setCreditTerms(e.target.value)}>{paymentOptions.map(p=><option key={p}>{p}</option>)}</select></Field>
        <Field label="Observação da decisão" wide><textarea rows={4} value={creditNotes} onChange={e=>setCreditNotes(e.target.value)} placeholder="Documentos analisados, justificativa, condição, ressalvas..."/></Field>
      </div>
      <p className="mt-4 text-xs leading-5 text-stone-500">A decisão não apaga a solicitação anterior. Cada nova revisão permanece registrada na trilha de crédito.</p>
      {error&&<div className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}
      <button disabled={saving} onClick={()=>void saveCredit()} className="mt-5 w-full rounded-xl bg-stone-950 py-3 text-sm font-semibold text-white disabled:opacity-50">{saving?"Salvando…":"Salvar decisão auditável"}</button>
      <CreditTimeline history={creditHistory}/>
    </div></div>}
  </div>;
}
...
function CreditRequestContext({history,loading}:{history:CreditEvent[];loading:boolean}){
  if(loading) return <div className="mt-5 rounded-2xl border bg-stone-50 p-4 text-xs text-stone-500">Carregando solicitação…</div>;
  const request=history.find(item=>item.eventType==="REQUEST");
  if(!request) return <div className="mt-5 rounded-2xl border border-amber-100 bg-amber-50 p-4"><p className="text-xs font-bold text-amber-900">Solicitação formal ainda não registrada</p><p className="mt-1 text-xs leading-5 text-amber-800">Este cliente entrou em análise pelo fluxo anterior. Você pode decidir agora; as próximas revisões já seguirão o rito auditável.</p></div>;
  return <div className="mt-5 rounded-2xl border border-violet-100 bg-violet-50/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-violet-700">Solicitação registrada</p><p className="mt-1 text-sm font-semibold">{money(Number(request.requestedLimit||0))} · {request.requestedTerms||"Condição não informada"}</p></div><span className="inline-flex items-center gap-1 text-[10px] text-violet-600"><Clock3 size={12}/>{dateTime(request.createdAt)}</span></div><p className="mt-2 text-xs leading-5 text-stone-600">{request.rationale}</p><p className="mt-2 text-[10px] text-stone-400">Solicitado por {request.actorName}{request.monthlyVolume?` · volume mensal estimado ${money(Number(request.monthlyVolume))}`:""}</p></div>;
}

function CreditTimeline({history}:{history:CreditEvent[]}){
  if(!history.length) return null;
  return <div className="mt-6 border-t pt-5"><div className="flex items-center gap-2"><History size={15} className="text-stone-500"/><h3 className="text-sm font-semibold">Trilha de crédito</h3></div><div className="mt-3 space-y-2">{history.slice(0,6).map(event=><div key={event.id} className="flex gap-3 rounded-xl bg-stone-50 px-3 py-3"><span className={`mt-1 size-2 shrink-0 rounded-full ${event.eventType==="REQUEST"?"bg-emerald-500":"bg-violet-500"}`}/><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-semibold">{event.eventType==="REQUEST"?"Crédito solicitado":`Decisão: ${event.decisionStatus?creditLabel[event.decisionStatus]:"registrada"}`}</p><span className="text-[10px] text-stone-400">{dateTime(event.createdAt)}</span></div><p className="mt-1 text-[11px] text-stone-500">{event.eventType==="REQUEST"?`${money(Number(event.requestedLimit||0))} · ${event.requestedTerms||"—"}`:`${money(Number(event.decidedLimit||0))} · ${event.decisionTerms||"—"}`} · {event.actorName}</p></div></div>)}</div></div>;
}

function HealthPanel({health,compact=false}:{health:Health;compact?:boolean}){return <div className={`mt-5 grid gap-2 ${compact?"grid-cols-2":"md:grid-cols-4"}`}><Mini label="Em aberto" value={money(health.openReceivables)}/><Mini label="Vencido" value={money(health.overdueAmount)} alert={health.overdueAmount>0}/><Mini label="Títulos vencidos" value={String(health.overdueCount)} alert={health.overdueCount>0}/><Mini label="Maior atraso" value={`${health.maxDaysOverdue} dias`} alert={health.maxDaysOverdue>0}/></div>;}
