...
type Dashboard = {
  ...
  historicoVendas: Array<{ label: string; value: number }>;
...
const emptyDraft = (): PriceDraft => ({
  price: "",
  maxRequestDiscountPercent: "0",
  maxApprovalDiscountPercent: "0",
  minimumPrice: "",
...
export default function CommercePage() {
  ...
  const variation = value.vendasPeriodoAnterior
    ? ((value.vendasUltimos30Dias - value.vendasPeriodoAnterior) /
        value.vendasPeriodoAnterior) *
      100
    : null;
  const visiblePrices =
    channelFilter === "ALL"
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#087568]">
            <Globe2 size={14} /> BBOS Commerce
          </p>
          <h1 className="mt-2 text-3xl font-bold">Commerce</h1>
          <p className="mt-2 text-sm text-[#626B69]">
            Canais, desempenho e política comercial conectados ao mesmo núcleo
            de preços.
          </p>
        </div>
      ...
      <section className="mt-6 rounded-2xl border border-[#E7E7E3] bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.14em] text-violet-700">
              <LockKeyhole size={13} /> Uso interno
            </p>
            <h2 className="mt-1 text-lg font-bold">
              Tabela de preços e governança comercial
            </h2>
            <p className="mt-1 max-w-3xl text-xs leading-5 text-[#626B69]">
              Esta é a fonte de verdade usada nos pedidos. O cliente vê apenas o
              preço final aprovado; limites, margem, ROI e aprovações permanecem
              internos.
            </p>
          </div>
          <select
            ...
            className="rounded-xl border border-[#E7E7E3] bg-white px-3 py-2 text-xs font-semibold"
          >
            ...
            <div className="divide-y divide-[#EFEFEB]">
              {visiblePrices.map((row) => {
                ...
                return (
                  <div
                    key={row.id}
                    className="grid grid-cols-[230px_110px_120px_120px_120px_120px_110px_80px] items-center gap-2 px-2 py-3"
                  >
                    <div className="min-w-0">
                      <p className="truncate text-xs font-bold text-[#202523]">
                        {row.productName} · {presentation(row.netWeightGrams)}
                      </p>
                      <p className="mt-1 truncate text-[10px] text-[#7A8381]">
                        {row.lineName} · {row.channelName} · {row.sku}
                      </p>
                    </div>
                    <PriceInput
                      value={draft.price}
                      onChange={(v) => updateDraft(row.id, "price", v)}
                      prefix="R$"
                    />
                    <PriceInput
                      value={draft.maxRequestDiscountPercent}
                      onChange={(v) =>
                        updateDraft(row.id, "maxRequestDiscountPercent", v)
                      }
                      suffix="%"
                    />
                    <PriceInput
                      value={draft.maxApprovalDiscountPercent}
                      onChange={(v) =>
                        updateDraft(row.id, "maxApprovalDiscountPercent", v)
                      }
                      suffix="%"
                    />
                    <PriceInput
                      value={draft.minimumPrice}
                      onChange={(v) => updateDraft(row.id, "minimumPrice", v)}
                      prefix="R$"
                      placeholder="—"
                    />
                    <PriceInput
                      ...
                      onChange={(v) =>
                        updateDraft(row.id, "minimumMarginPercent", v)
                      }
                      suffix="%"
                      placeholder="—"
                    />
                    <PriceInput
                      value={draft.minimumRoiPercent}
                      onChange={(v) =>
                        updateDraft(row.id, "minimumRoiPercent", v)
                      }
                      suffix="%"
                      placeholder="—"
                    />
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1 rounded-xl bg-[#14201D] px-3 py-2.5 text-[10px] font-bold text-white disabled:opacity-50"
                    >
                      <Save size={12} />
                      {saving === row.id ? "..." : "Salvar"}
                    </button>
                  </div>
                );
              })}
        ...
        <div className="mt-4 rounded-xl bg-[#F7F8F6] px-4 py-3 text-[10px] leading-5 text-[#626B69]">
          <strong>Regra:</strong> o máximo aprovável nunca pode ser maior que o
          máximo solicitável. O pedido também respeita preço mínimo, margem
          mínima e ROI mínimo antes de permitir qualquer solicitação de
          desconto.
        </div>
      </section>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 text-lg font-bold">Produtos mais vendidos</h2>
            </div>
            <Link href="/produtos" className="text-xs font-bold text-[#087568]">
          ...
          {value.produtosMaisVendidos.length ? (
            <div className="mt-4 space-y-3">
              {value.produtosMaisVendidos.map((item, index) => (
                <Link
                  href={`/vendas/produtos/${item.id}`}
                  key={item.id}
                  className="flex items-center gap-3 rounded-xl border border-[#E7E7E3] p-3 transition hover:bg-[#F7F9F8]"
                >
                  <span className="grid size-7 place-items-center rounded-lg bg-[#F0F0ED] text-xs font-bold text-[#087568]">
                    {index + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold">
                      {item.product}{" "}
                      <span className="font-medium text-[#626B69]">
                        {item.presentation}
                      </span>
                    </p>
                    <div className="mt-1 h-1 rounded-full bg-[#E7E7E3]">
                      <div
                        ...
                        style={{
                          width: `${Math.min((item.revenue / Math.max(value.vendasUltimos30Dias, 1)) * 100, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 text-lg font-bold">
                Estoque que merece atenção
              </h2>
            </div>
            <Link href="/estoque" className="text-xs font-bold text-[#087568]">
              Ver estoque <ArrowRight className="inline" size={13} />
            </Link>
          </div>
          <div className="mt-4 rounded-xl border border-dashed border-[#E7E7E3] p-5 text-center">
            <TriangleAlert className="mx-auto text-[#E99A35]" size={20} />
            <p className="mt-2 text-xs font-semibold">
              A análise por canal será ativada com o histórico de pedidos
              online.
            </p>
            <p className="mt-1 text-[10px] text-[#7A8381]">
              O saldo continua sendo o mesmo estoque físico, reservado e
              disponível do BBOS.
            </p>
          </div>
        </Card>
...
function toDraft(row: PriceRow): PriceDraft {
  return {
    ...
    minimumMarginPercent:
      row.minimumMarginPercent == null ? "" : String(row.minimumMarginPercent),
    minimumRoiPercent:
      row.minimumRoiPercent == null ? "" : String(row.minimumRoiPercent),
  };
}
...
function PriceInput({
  ...
  return (
    <label className="flex items-center rounded-xl border border-[#E7E7E3] bg-white px-2.5 py-2">
      <span className="mr-1 text-[9px] font-semibold text-[#7A8381]">
        {prefix}
      </span>
      <input
        value={value}
      ...
      />
      <span className="ml-1 text-[9px] font-semibold text-[#7A8381]">
        {suffix}
      </span>
    </label>
  );
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2">
        <TriangleAlert size={16} className="text-[#E99A35]" />
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#E99A35]">
            Operação comercial
          </p>
          <h2 className="mt-1 text-lg font-bold">
      ...
      {items.length ? (
        ...
        <EmptyState text="Nenhum pedido exige atenção neste momento." />
      )}
    </Card>
  );
}
function SalesChart({
  ...
  const path = points
    .map(
      (p, i) =>
        `${i * (280 / Math.max(points.length - 1, 1)) + 20},${112 - (p.value / max) * 85}`,
    )
    .join(" ");
  return (
    <div className="mt-5">
      <svg
        ...
        role="img"
        aria-label="Evolução das vendas online"
      >
        <line x1="20" x2="300" y1="112" y2="112" stroke="#E7E7E3" />
        <polyline
          points={path}
          fill="none"
          stroke="#087568"
        ...
        {points.map((p, i) => (
          <text
            key={p.label}
            x={i * (280 / Math.max(points.length - 1, 1)) + 20}
            y="132"
            textAnchor="middle"
            fontSize="9"
            fill="#7A8381"
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="mt-5 rounded-xl border border-dashed border-[#E7E7E3] p-6 text-center text-xs text-[#7A8381]">
      <BarChart3 className="mx-auto mb-2 text-[#B5C1BD]" size={19} />
      {text}
    </div>
  );
}
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-[10px] font-semibold text-[#626B69]">{label}</p>
