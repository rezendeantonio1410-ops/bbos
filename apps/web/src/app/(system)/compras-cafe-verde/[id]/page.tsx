...
export default function PurchaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-[1180px]">
    ...
    <header className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-forest-700">Ficha completa da compra</p><h1 className="mt-2 text-3xl font-bold">{purchase.purchaseNumber}</h1><p className="mt-2 text-sm text-stone-500">{purchase.supplier.name} · {purchase.supplier.taxId ?? "Documento não informado"}</p></div><div className="flex flex-wrap gap-2"><Badge>{approval[purchase.approvalStatus] ?? purchase.approvalStatus}</Badge><Badge>{operationalLabel}</Badge><Badge>{financial[purchase.financialStatus] ?? purchase.financialStatus}</Badge><Badge>{external[purchase.externalAcceptanceStatus] ?? purchase.externalAcceptanceStatus}</Badge></div></header>
    <Card className="mt-6 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-forest-700">Ações da ficha</p><p className="mt-1 text-sm text-stone-500">A aprovação, o aceite e o recebimento são etapas independentes.</p></div><div className="flex flex-wrap gap-2">
      ...
      {purchase.approvalStatus === "APPROVED" && purchase.externalAcceptanceStatus === "ACCEPTED" && <Link className="inline-flex min-h-11 items-center rounded-xl border px-4 py-2.5 text-sm font-semibold text-forest-800" href={`/recebimento?purchase=${purchase.id}`}>Registrar recebimento</Link>}
      {purchase.approvalStatus === "APPROVED" && purchase.externalAcceptanceStatus === "ACCEPTED" && <a className="inline-flex min-h-11 items-center rounded-xl border px-4 py-2.5 text-sm font-semibold text-forest-800" href={`${API_ROOT}/green-coffee-purchases/${purchase.id}/contract.pdf`}>Baixar contrato</a>}
    </div></div></Card>
    ...
    <Card className="mt-4 p-5"><h2 className="text-xs font-bold uppercase tracking-[.13em] text-forest-700">Rastreabilidade</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><TimelineItem label="Compra criada" state="Concluída" /><TimelineItem label="Aprovação interna" state={approval[purchase.approvalStatus] ?? purchase.approvalStatus} /><TimelineItem label="Aceite fornecedor" state={external[purchase.externalAcceptanceStatus] ?? purchase.externalAcceptanceStatus} /><TimelineItem label="Recebimento" state={purchase.receivedKg > 0 ? (purchase.balanceKg > 0 ? "Parcial" : "Completo") : "Pendente"} /><TimelineItem label="Laboratório / estoque" state={purchase.receivedKg > 0 ? "Em acompanhamento" : "Pendente"} /></div></Card>
  </div>;
}
...
function TimelineItem({ label, state }: { label: string; state: string }) { return <div className="rounded-xl border border-stone-100 bg-white p-3"><div className="flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded-full bg-forest-50 text-forest-700"><Check size={14} /></span><p className="text-sm font-semibold">{label}</p></div><p className="mt-2 pl-8 text-xs text-stone-500">{state}</p></div>; }
