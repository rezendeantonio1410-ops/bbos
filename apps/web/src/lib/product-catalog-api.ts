...
export const PRODUCTS_API_URL = `${getApiBaseUrl()}/products`;
...
export async function loadProductCatalog(): Promise<{
  ...
  if (!response.ok)
    throw new Error("Não foi possível carregar o catálogo persistente.");
...
export async function loadProduct(id: string): Promise<CatalogProduct | null> {
  ...
  if (!response.ok)
    throw new Error("Não foi possível carregar o produto persistente.");
