...
function localAnswer(question: string, context: ReturnType<typeof getContext>) {
  ...
  if (q.includes("margem") || q.includes("roi") || q.includes("caixa")) return "Eu separo faturamento, margem, caixa e ROI porque representam decisões diferentes. Só recomendo uma ação quando a origem do número estiver rastreável no BBOS.";
...
export function BbosAssistant() {
  ...
  React.useEffect(() => {
    ...
    const interceptIntelligenceLinks = (event: MouseEvent) => {
      ...
      if (href === "/bi" && (label.includes("ia") || label.includes("inteligência") || label.includes("intelligence"))) {
    ...
    return () => { window.removeEventListener("bbos:open-assistant", openAssistant); document.removeEventListener("click", interceptIntelligenceLinks); };
  }, []);
  ...
  return <>
    ...
    {open && <div className="bbos-ai-overlay" role="dialog" aria-modal="true" aria-label="BBOS Intelligence">
      ...
      <aside className="bbos-ai-panel">
        ...
        <div className="bbos-ai-conversation">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`bbos-ai-message bbos-ai-message-${message.role}`}>{message.text}</div>)}</div>
