...
const compositionImages: Record<string, string[]> = {
  Frutado: [
    ...
    "/sensory/aroma/frutado/frutas-vermelhas/mirtilo.png",
  ],
};
...
function subfamilyPath(index: number, count: number) {
  const center = [205, 214] as const; const inner = 94; const outer = 202; const start = 112 + index * 136 / count + 1.2; const end = 112 + (index + 1) * 136 / count - 1.2;
...
export function SensoryReferenceFlow({ context, families, value, onChange, mode, onDepthChange, onComplete }: Props) {
  ...
  return <div className="mx-auto w-full max-w-[390px] overflow-visible bg-[#fbfaf7] text-[#211b18]" data-reference-sensory-flow data-mode={mode}>
